...
{
	//verify that look at pos is not the same as cam pos. thy cannot be the same as that wouldn't make sense and would result in undefined behaviour.
	if (lookAtPos.x == pos.x && lookAtPos.y == pos.y && lookAtPos.z == pos.z) { return; }
