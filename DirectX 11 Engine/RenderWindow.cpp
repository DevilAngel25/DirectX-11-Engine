...
bool RenderWindow::Initialize(WindowContainer * pWindowContainer, HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height)
{
	...
	this->handle = CreateWindowEx(0,
		this->window_class_wide.c_str(), // windows class name
		this->window_title_wide.c_str(), //windows type
		WS_CAPTION | WS_MINIMIZEBOX | WS_SYSMENU, //windows style
		wr.left, //windows x position
		wr.top, //windows y position
		wr.right - wr.left, //windows width
		wr.bottom - wr.top, //windows height
...
LRESULT CALLBACK HandleMessageSetup(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NCCREATE:
	{
		...
		if (pWindow == nullptr)
		{
			ErrorLogger::Log("Critical Error: Pointer to windows container is null during WM_NCCREATE.");
			exit(-1);
		}
